...
def buscar_estado_projeto(project_id):
    projeto = supabase.table("projects") \
    .select("memory_summary") \
    .eq("id", project_id) \
    .execute()
    ultimo_prompt = supabase.table("prompts") \
    .select("content") \
    .eq("project_id", project_id) \
    .order("created_at", desc=True) \
    .limit(1) \
    .execute()
    memoria= projeto.data[0]["memory_summary"]
    ultimo = ultimo_prompt.data[0]["content"] if ultimo_prompt.data else "Sem histórico de prompts."
    return memoria, ultimo

def criar_projeto(titulo):
    data={
        "title": titulo,
    }
    resposta = supabase.table("projects").insert(data).execute() #Salva o titulo do projeto no banco de dados.
    return resposta.data[0]["id"]

def salvar_prompt(project_id, content):
    data={
        "project_id":project_id,
        "content": content
    }
    supabase.table("prompts").insert(data).execute() #Salva prompt gerado no banco de dados na coluna prompts da tabela projects

def buscar_contexto(project_id):
    resposta = supabase.table("prompts") \
    .select("content") \
    .eq("project_id", project_id) \
    .execute()
    textos = [item["content"] for item in resposta.data]
    return "\n\n".join(textos)

def gerar_incremento(project_id, nova_instrucao):
    memoria,ultimo_prompt = buscar_estado_projeto(project_id)


    prompt = f"""
Você está evoluindo um projeto em etapas.

Resumo consolidado do projeto:
{memoria}

Última parte desenvolvida:
{ultimo_prompt}

Nova solicitação:
{nova_instrucao}

Gere apenas a nova parte.
Não reescreva o conteúdo anterior.
Mantenha coerência absoluta.
"""
    ...
    nova_parte = resposta.text
    salvar_prompt(project_id, nova_parte)
    atualizar_memoria(project_id,memoria, nova_parte)

    return nova_parte
def atualizar_memoria(project_id, memoria_atual, nova_parte):
    prmpt_resumo = f"""
